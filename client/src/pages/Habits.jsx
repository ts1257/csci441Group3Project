...
function HabitsContent() {
  ...
  const loadHabits = async () => {
    try {
      ...
      if (!response.ok) throw new Error(data.message || "Failed to load habits");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        setHabits((prev) => prev.map((habit) => (habit._id === savedHabit._id ? savedHabit : habit)));
      } else {
  ...
  return (
    <>
      <div className="mb-6 grid gap-4 md:grid-cols-3">
        ...
        <StatCard label="Reminder Times" value={loading ? "..." : stats.reminders} />
      </div>
      ...
      <section className="rounded-4xl border border-base-300 bg-base-100 p-5 shadow-sm md:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm opacity-70">Create habits here. Daily completion is shown on the Wellness dashboard.</p>
          </div>
        ...
        {loading ? (
          ...
          <p className="mt-6 opacity-70">No habits yet. Add your first wellness habit.</p>
        ) : (
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            {habits.map((habit) => (
              <article key={habit._id} className="rounded-3xl border border-base-300 p-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-semibold">{habit.name}</h3>
                    <p className="mt-1 text-sm capitalize opacity-70">{habit.category?.replace("-", " ")}</p>
                  </div>
                  <span className={`badge ${habit.completedToday ? "badge-success" : "badge-outline"}`}>
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  <button className="btn btn-outline btn-sm rounded-xl" onClick={() => addOneUnit(habit)}>
                    +1 {habit.unit || "unit"}
                  </button>
                  <button className="btn btn-outline btn-sm rounded-xl" onClick={() => subtractOneUnit(habit)}>
                    -1 {habit.unit || "unit"}
                  </button>
                  <button className="btn btn-sm rounded-xl" onClick={() => toggleCompleted(habit)}>
                    {habit.completedToday ? "Mark Not Done" : "Complete Today"}
                  </button>
                  <button className="btn btn-outline btn-sm rounded-xl" onClick={() => startEdit(habit)}>
                    Edit
                  </button>
                  <button className="btn btn-error btn-outline btn-sm rounded-xl" onClick={() => deleteHabit(habit)}>
                    Delete
                  </button>
                </div>
              </article>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-4xl bg-base-100 p-5 shadow-xl md:p-6">
            <div className="mb-5 flex items-center justify-between gap-4">
              <h2 className="text-2xl font-bold">{editingId ? "Edit Habit" : "Add Habit"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" name="name" value={form.name} onChange={handleChange} className="input input-bordered w-full rounded-2xl" placeholder="Drink water" />
              </div>

              <div>
                <label className="mb-2 block text-sm font-medium">Category</label>
                <select name="category" value={form.category} onChange={handleChange} className="select select-bordered w-full rounded-2xl">
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <div>
                  ...
                  <input type="text" name="goal" value={form.goal} onChange={handleChange} className="input input-bordered w-full rounded-2xl" placeholder="8" />
                </div>
                <div>
                  <label className="mb-2 block text-sm font-medium">Progress</label>
                  <input type="text" name="progress" value={form.progress} onChange={handleChange} className="input input-bordered w-full rounded-2xl" placeholder="4" />
                </div>
                <div>
                  <label className="mb-2 block text-sm font-medium">Unit</label>
                  <input type="text" name="unit" value={form.unit} onChange={handleChange} className="input input-bordered w-full rounded-2xl" placeholder="cups, pages, minutes" />
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input type="time" name="reminderTime" value={form.reminderTime} onChange={handleChange} className="input input-bordered w-full rounded-2xl" />
                </div>
                <label className="flex items-center gap-3 self-end rounded-2xl border border-base-300 px-4 py-3">
                  <input type="checkbox" name="completedToday" checked={form.completedToday} onChange={handleChange} className="checkbox checkbox-primary" />
              ...
              <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:justify-end">
                ...
                <button type="submit" className="btn btn-primary rounded-2xl" disabled={submitting}>{submitting ? "Saving..." : editingId ? "Save Changes" : "Add Habit"}</button>
              </div>
            </form>
          </div>
        </div>
